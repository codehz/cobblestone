...
bool ScriptTemplateFactory<ScriptServerContext>::Component::retrieveComponentFrom(ScriptApi::ScriptVersionInfo const &version, ScriptEngine &engine,
                                                                                  ScriptServerContext &ctx,
  ...
  if (auto binder = scriptengine->helpDeserialize(source); binder) {
    if (binder->hasComponent<ScriptActorUniqueIdBinderComponent>()) {
      ...
      if (actor) return retrieveComponentFrom(version, engine, ctx, *actor, target);
      return false;
    } else if (binder->hasComponent<ScriptLevelBinderComponent>()) {
      Level *level = nullptr;
      QCHECK(scriptengine->helpGetLevel(source, &level));
      if (level) return retrieveComponentFrom(version, engine, ctx, *level, target);
      return false;
    } else if (auto pos_comp = binder->getComponent<ScriptBlockPositionBinderComponent>();
               pos_comp && binder->hasComponent<ScriptTickingAreaBinderComponent>()) {
...
bool ScriptTemplateFactory<ScriptServerContext>::Component::hasComponent(ScriptApi::ScriptVersionInfo const &version, ScriptEngine &engine,
                                                                         ScriptServerContext &ctx, ScriptApi::ScriptObjectHandle const &source,
                                                                         bool &target) const {
  if (auto binder = scriptengine->helpDeserialize(source); binder) {
    if (binder->hasComponent<ScriptActorUniqueIdBinderComponent>()) {
      Actor *actor = nullptr;
      QCHECK(scriptengine->helpGetActor(source, &actor));
      if (actor) return hasComponent(version, engine, ctx, *actor, target);
    } else if (binder->hasComponent<ScriptLevelBinderComponent>()) {
      Level *level = nullptr;
      QCHECK(scriptengine->helpGetLevel(source, &level));
      if (level) return hasComponent(version, engine, ctx, *level, target);
    } else if (auto pos_comp = binder->getComponent<ScriptBlockPositionBinderComponent>();
               pos_comp && binder->hasComponent<ScriptTickingAreaBinderComponent>()) {
      BlockSource *blocksource;
      QCHECK(scriptengine->helpGetBlockSource(source, &blocksource));
