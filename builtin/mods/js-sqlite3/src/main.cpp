...
static JSValue createSQLite3(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
  ...
  if (res != SQLITE_OK)
    return JS_ThrowInternalError(ctx, "Failed to open database: %s", sqlite3_errstr(res));
...
static JSValue processExecute(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
  if (auto db = (sqlite3 *)JS_GetOpaque(this_val, sqlite3_class); db) {
    if (argc == 1) {
      std::string stmt;
      if (!scriptengine->getValue(argv[0], stmt))
        return JS_ThrowTypeError(ctx, "Require (string)");
      char *errmsg;
      ...
      std::string stmt;
      if (!scriptengine->getValue(argv[0], stmt) || !JS_IsFunction(js_context, argv[1]))
        return JS_ThrowTypeError(ctx, "Require (string, function)");
      auto cbn = argv[1];
      auto callback = [&](int n, char **values, char **keys) -> bool {
        autoval val = JS_NewObject(js_context);
        for (int i = 0; i < n; i++) {
          JS_SetPropertyStr(js_context, val, keys[i], JS_NewString(js_context, values[i]));
        }
        autoval result = JS_Call(js_context, cbn, JS_UNDEFINED, 1, &val);
        if (!handle_exception(js_context, "SQLite3::exec"))
          return false;
        if (JS_IsBool(result) && !JS_ToBool(js_context, result))
          return false;
        return true;
      };
      char *errmsg;
...
static void fill_stmt(JSContext *ctx, sqlite3 *db, sqlite3_stmt *stmt, JSValue &obj) {
  autoval global = JS_GetGlobalObject(ctx);
  autoval reflect = JS_GetPropertyStr(ctx, global, "Reflect");
  autoval ownKeys = JS_GetPropertyStr(ctx, reflect, "ownKeys");
  autoval arr = JS_Call(ctx, ownKeys, reflect, 1, &obj);
  autoval propLen = JS_GetPropertyStr(ctx, arr, "length");
  for (int32_t i = 0; i < JS_VALUE_GET_INT(propLen); i++) {
    autoval key = JS_GetPropertyUint32(ctx, arr, i);
    autostr keyStr = JS_ToCString(ctx, key);
    autoval value = JS_GetPropertyStr(ctx, obj, keyStr);
...
static JSValue processQuery(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
  if (auto db = (sqlite3 *)JS_GetOpaque(this_val, sqlite3_class); db) {
    if (argc < 1)
      return JS_ThrowTypeError(ctx, "Require 1 or 2 arguments");
    std::string sql;
    if (!scriptengine->getValue(argv[0], sql))
    ...
    if (argc > 1) {
      ...
      } catch (JSValue const &v) {
        return v;
      }
    }
    int ret = 0;
    int idx = 0;
    autohandle arr = JS_NewArray(ctx);
    while ((ret = sqlite3_step(stmt)) == SQLITE_ROW) {
      autohandle obj = JS_NewObject(ctx);
      ...
      for (int i = 0; i < count; i++) {
        ...
        switch (vtype) {
        case SQLITE_INTEGER:
          value = JS_NewInt64(ctx, sqlite3_column_int64(stmt, i));
          break;
        case SQLITE_FLOAT:
          value = JS_NewFloat64(ctx, sqlite3_column_double(stmt, i));
          break;
        case SQLITE_TEXT:
          value = JS_NewString(ctx, (char const *)sqlite3_column_text(stmt, i));
          break;
        case SQLITE_BLOB:
          value = JS_NewArrayBufferCopy(ctx, (uint8_t *)sqlite3_column_blob(stmt, i), sqlite3_column_bytes(stmt, i));
          break;
        case SQLITE_NULL:
          value = JS_NULL;
        default:
...
static JSValue processUpdate(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
  if (auto db = (sqlite3 *)JS_GetOpaque(this_val, sqlite3_class); db) {
    ...
    case SQLITE_ROW:
      return JS_ThrowSyntaxError(ctx, "Cannot perform query in this context");
    default:
      return JS_ThrowInternalError(ctx, "Failed to serialize sqlite result table: %s", sqlite3_errmsg(db));
    }
  }
  return JS_ThrowTypeError(ctx, "Require SQLite3 object");
}
